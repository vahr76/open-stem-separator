...
La carpeta predeterminada es Music/OSS/downloads dentro del directorio personal. Se puede cambiar con `configure` o con `--output` en modo avanzado. Antes de ejecutar el motor se crea la carpeta si corresponde y se prueba la escritura con un archivo temporal. Si falla, se informa el problema sin elevar privilegios. Esta prueba no garantiza espacio libre durante toda la descarga ni evita cambios de permisos posteriores.

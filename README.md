...
Cada descarga crea una carpeta de proyecto. OSS intenta detectar `artista/tema` desde metadata real de yt-dlp o desde títulos con forma `Artista - Tema`. Si no hay confianza suficiente, guarda en `_unsorted/Titulo`. También escribe `metadata.json` con URL, título, uploader, duración, ID y el origen/confianza de la detección, además de un `project.json` inicial que funcionará como manifiesto estable del proyecto OSS. Cuando una descarga termina correctamente, `project.json` registra los archivos multimedia encontrados en `media`.

...
DEFAULT_INSTALL_DIR="${HOME}/Library/Application Support/OSS"
...
download_if_missing() {
  ...
  if ! command -v yt-dlp >/dev/null 2>&1 && [ ! -x "$install_dir/bin/yt-dlp" ]; then
    printf '%s\n' "Descargando yt-dlp..."
    curl -L --fail --output "$install_dir/bin/yt-dlp" "https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp_macos"
    chmod +x "$install_dir/bin/yt-dlp"
  fi
  if ! command -v ffmpeg >/dev/null 2>&1 || ! command -v ffprobe >/dev/null 2>&1; then
    printf '%s\n' "FFmpeg/ffprobe no están en PATH. Instalalos con Homebrew o agregalos a ${install_dir}/bin."
  fi
